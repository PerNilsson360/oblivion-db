...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == token::TOK_END || tok == token::TOK_LSQUARE || tok == token::TOK_RSQUARE || tok == token::TOK_LCURL || tok == token::TOK_RCURL || tok == token::TOK_COMMA || tok == token::TOK_COLON || tok == token::TOK_FALSE || tok == token::TOK_NULL || tok == token::TOK_TRUE);
      }
#else
      ...
      symbol_type (int tok, std::string v, location_type l)
        : super_type(token_type (tok), std::move (v), std::move (l))
      {
        YY_ASSERT (tok == token::TOK_STRING);
      }
#else
    ...
    JsonParser (JsonDriver& driver_yyarg);
    virtual ~JsonParser ();
...
#if 201103L <= YY_CPLUSPLUS
  template <typename Base>
  JsonParser::basic_symbol<Base>::basic_symbol (basic_symbol&& that)
    : Base (std::move (that))
    , value ()
...
#if 201103L <= YY_CPLUSPLUS
  inline
  JsonParser::by_type::by_type (by_type&& that)
    : type (that.type)
  {
    that.clear ();
  }
  ...
  inline
  JsonParser::by_type::by_type (const by_type& that)
    : type (that.type)
  {}
  ...
  inline
  int
  JsonParser::by_type::type_get () const YY_NOEXCEPT
  {
    return type;
  }

} // yy
#line 1445 "JsonParser.tab.hh"
